# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00_Fastai/00_torch_core.ipynb.

# %% auto 0
__all__ = ['NoneType', 'string_classes', 'TEST_IMAGE_BW', 'TEST_IMAGE', 'norm_types', 'is_iter', 'is_coll', 'all_equal', 'noop',
           'noops', 'one_is_instance', 'equals', 'pv', 'setup_cuda', 'subplots', 'show_image', 'show_titled_image',
           'show_images', 'ArrayBase', 'ArrayImageBase', 'ArrayImage', 'ArrayImageBW', 'ArrayMask', 'evaluating',
           'tensor', 'set_seed', 'get_random_states', 'set_random_states', 'no_random', 'unsqueeze', 'unsqueeze_',
           'apply', 'maybe_gather', 'to_detach', 'to_half', 'to_float', 'default_device', 'to_device', 'to_cpu',
           'to_np', 'to_concat', 'TensorBase', 'TensorImageBase', 'TensorImage', 'TensorImageBW', 'TensorMask',
           ...
           'get_model', 'one_hot', 'one_hot_decode', 'params', 'trainable_params', 'norm_bias_params',
           'batch_to_samples', 'logit', 'num_distrib', 'rank_distrib', 'distrib_barrier', 'base_doc', 'doc',
           'nested_reorder', 'flatten_check', 'make_cross_image', 'show_image_batch', 'requires_grad', 'init_default',
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 3
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 6
_all_ = ['progress_bar','master_bar']

# %% ../nbs/00_Fastai/00_torch_core.ipynb 7
defaults.benchmark = True

# %% ../nbs/00_Fastai/00_torch_core.ipynb 8
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 10
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 13
def _fig_bounds(x):
    r = x//32
    return min(5, max(1,r))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 14
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 21
@delegates(show_image, keep=True)
def show_titled_image(o, **kwargs):
    "Call `show_image` destructuring `o` to `(img,title)`"
    show_image(o[0], title=str(o[1]), **kwargs)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 24
@delegates(subplots)
...
def evaluating(model):
    ...
    try:
        ...
        if isinstance(model,tuple):
            for was_training,m in zip(istrain,model):
                if was_training:
                    m.train()
        elif istrain:
            model.train()

# %% ../nbs/00_Fastai/00_torch_core.ipynb 38
@patch
def __array_eq__(self:Tensor,b):
    return torch.equal(self,b) if self.dim() else self==b

# %% ../nbs/00_Fastai/00_torch_core.ipynb 39
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 40
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 43
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 48
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 49
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 54
@contextmanager
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 60
def unsqueeze(x, dim=-1, n=1):
    "Same as `torch.unsqueeze` but can add `n` dims"
    for _ in range(n): x = x.unsqueeze(dim)
    return x

# %% ../nbs/00_Fastai/00_torch_core.ipynb 62
def unsqueeze_(x, dim=-1, n=1):
    "Same as `torch.unsqueeze_` but can add `n` dims"
    for _ in range(n): x.unsqueeze_(dim)
    return x

# %% ../nbs/00_Fastai/00_torch_core.ipynb 64
def _fa_rebuild_tensor (cls, *args, **kwargs): return cls(torch._utils._rebuild_tensor_v2(*args, **kwargs))
def _fa_rebuild_qtensor(cls, *args, **kwargs): return cls(torch._utils._rebuild_qtensor  (*args, **kwargs))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 65
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 66
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 67
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 69
def to_half(b):
    "Recursively map lists of tensors in `b ` to FP16."
    return apply(lambda x: x.half() if torch.is_floating_point(x) else x, b)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 70
def to_float(b):
    "Recursively map lists of int tensors in `b ` to float."
    return apply(lambda x: x.float() if torch.is_floating_point(x) else x, b)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 71
# None: True if available; True: error if not available; False: use CPU
defaults.use_cuda = None

# %% ../nbs/00_Fastai/00_torch_core.ipynb 72
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 73
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 75
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 78
def to_cpu(b):
    "Recursively map lists of tensors in `b ` to the cpu."
    return to_device(b,'cpu')

# %% ../nbs/00_Fastai/00_torch_core.ipynb 80
def to_np(x):
    "Convert a tensor to a numpy array."
    return apply(lambda o: o.data.cpu().numpy(), x)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 82
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 86
@patch
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 87
if not hasattr(torch,'as_subclass'): torch.as_subclass = torch.Tensor.as_subclass

# %% ../nbs/00_Fastai/00_torch_core.ipynb 88
@patch
def as_subclass(self:Tensor, typ):
    "Cast to `typ` and include `__dict__` and meta"
    return retain_meta(self, torch.as_subclass(self, typ))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 91
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 92
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 106
class TensorImageBase(TensorBase):
    _show_args = ArrayImageBase._show_args
    def show(self, ctx=None, **kwargs):
        return show_image(self, ctx=ctx, **{**self._show_args, **kwargs})

# %% ../nbs/00_Fastai/00_torch_core.ipynb 107
class TensorImage(TensorImageBase): pass

# %% ../nbs/00_Fastai/00_torch_core.ipynb 108
class TensorImageBW(TensorImage): _show_args = ArrayImageBW._show_args

# %% ../nbs/00_Fastai/00_torch_core.ipynb 109
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 110
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 133
def concat(*ls):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 135
class Chunks:
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 140
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 142
class ShowTitle:
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 149
@patch
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 153
def get_empty_df(n):
    "Return `n` empty rows of a dataframe"
    df = pd.DataFrame(index = range(n))
    return [df.iloc[i] for i in range(n)]

# %% ../nbs/00_Fastai/00_torch_core.ipynb 154
def display_df(df):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 155
def get_first(c):
    "Get the first element of c, even if c is a dataframe"
    return getattr(c, 'iloc', c)[0]

# %% ../nbs/00_Fastai/00_torch_core.ipynb 156
def one_param(m):
    "First parameter in `m`"
    return first(m.parameters())

# %% ../nbs/00_Fastai/00_torch_core.ipynb 157
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 158
def find_device(b):
    "Recursively search the device of `b`."
    return item_find(b).device

# %% ../nbs/00_Fastai/00_torch_core.ipynb 160
def find_bs(b):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 162
def np_func(f):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 166
class Module(nn.Module, metaclass=PrePostInitMeta):
    "Same as `nn.Module`, but no need for subclasses to call `super().__init__`"
    def __pre_init__(self, *args, **kwargs): super().__init__()
    def __init__(self): pass

# %% ../nbs/00_Fastai/00_torch_core.ipynb 169
from torch.nn.parallel import DistributedDataParallel

# %% ../nbs/00_Fastai/00_torch_core.ipynb 170
def get_model(model):
    "Return the model maybe wrapped inside `model`."
    return model.module if isinstance(model, (DistributedDataParallel, nn.DataParallel)) else model

# %% ../nbs/00_Fastai/00_torch_core.ipynb 171
def one_hot(x, c):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 173
def one_hot_decode(x, vocab=None):
    return L(vocab[i] if vocab else i for i,x_ in enumerate(x) if x_==1)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 175
def params(m):
    "Return all parameters of `m`"
    return [p for p in m.parameters()]

# %% ../nbs/00_Fastai/00_torch_core.ipynb 176
def trainable_params(m):
    "Return all trainable parameters of `m`"
    return [p for p in m.parameters() if p.requires_grad]

# %% ../nbs/00_Fastai/00_torch_core.ipynb 178
norm_types = (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d, nn.LayerNorm)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 179
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 181
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 183
@patch
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 185
@patch
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 186
def logit(x):
    "Logit of `x`, clamped to avoid inf."
    x = x.clamp(1e-7, 1-1e-7)
    return -(1/x-1).log()

# %% ../nbs/00_Fastai/00_torch_core.ipynb 187
def num_distrib():
    "Return the number of processes in distributed training (if applicable)."
    return int(os.environ.get('WORLD_SIZE', 0))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 188
def rank_distrib():
    "Return the distributed rank of this process (if applicable)."
    return int(os.environ.get('RANK', 0))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 189
def distrib_barrier():
    "Place a synchronization barrier in distributed training"
    if num_distrib() > 1 and torch.distributed.is_initialized(): torch.distributed.barrier()

# %% ../nbs/00_Fastai/00_torch_core.ipynb 191
# Saving arrays requires pytables - optional dependency
try: import tables
except: pass

# %% ../nbs/00_Fastai/00_torch_core.ipynb 192
def _comp_filter(lib='lz4',lvl=3): return tables.Filters(complib=f'blosc:{lib}', complevel=lvl)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 193
@patch
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 195
@patch
def load_array(p:Path):
    "Save numpy array to a `pytables` file"
    with tables.open_file(p, 'r') as f: return f.root.data.read()

# %% ../nbs/00_Fastai/00_torch_core.ipynb 196
def base_doc(elt):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 197
def doc(elt):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 198
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 200
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 203
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 206
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 209
def requires_grad(m):
    "Check if the first parameter of `m` requires grad or not"
    ps = list(m.parameters())
    return ps[0].requires_grad if len(ps)>0 else False

# %% ../nbs/00_Fastai/00_torch_core.ipynb 211
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 213
def cond_init(m, func):
    "Apply `init_default` to `m` unless it's a batchnorm module"
    if (not isinstance(m, norm_types)) and requires_grad(m): init_default(m, func)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 215
def apply_leaf(m, f):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 217
def apply_init(m, func=nn.init.kaiming_normal_):
    "Initialize all non-batchnorm layers of `m` with `func`."
    apply_leaf(m, partial(cond_init, func=func))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 220
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 221
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 222
def script_fwd(*argidx):
    "Decorator: create static jit script and save args with indices `argidx` using `ctx.save_for_backward`"
    return script_save_ctx(True, *argidx)

# %% ../nbs/00_Fastai/00_torch_core.ipynb 223
def script_bwd(f):
    "Decorator: create static jit script and pass everything in `ctx.saved_variables to `f`, after `*args`"
    return staticmethod(script_use_ctx(f))

# %% ../nbs/00_Fastai/00_torch_core.ipynb 224
def grad_module(cls):
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 226
...
# %% ../nbs/00_Fastai/00_torch_core.ipynb 227
