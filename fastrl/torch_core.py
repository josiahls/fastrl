# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/Fastai/00_torch_core.ipynb.
...
# %% ../nbs/Fastai/00_torch_core.ipynb 2
...
# %% ../nbs/Fastai/00_torch_core.ipynb 5
_all_ = ['progress_bar','master_bar']

# %% ../nbs/Fastai/00_torch_core.ipynb 6
defaults.benchmark = True

# %% ../nbs/Fastai/00_torch_core.ipynb 7
...
# %% ../nbs/Fastai/00_torch_core.ipynb 10
...
# %% ../nbs/Fastai/00_torch_core.ipynb 13
def _fig_bounds(x):
    r = x//32
    return min(5, max(1,r))

# %% ../nbs/Fastai/00_torch_core.ipynb 14
...
# %% ../nbs/Fastai/00_torch_core.ipynb 21
@delegates(show_image, keep=True)
def show_titled_image(o, **kwargs):
    "Call `show_image` destructuring `o` to `(img,title)`"
    show_image(o[0], title=str(o[1]), **kwargs)

# %% ../nbs/Fastai/00_torch_core.ipynb 24
@delegates(subplots)
...
# %% ../nbs/Fastai/00_torch_core.ipynb 39
...
# %% ../nbs/Fastai/00_torch_core.ipynb 42
...
# %% ../nbs/Fastai/00_torch_core.ipynb 47
...
# %% ../nbs/Fastai/00_torch_core.ipynb 48
...
# %% ../nbs/Fastai/00_torch_core.ipynb 53
@contextmanager
...
# %% ../nbs/Fastai/00_torch_core.ipynb 59
def unsqueeze(x, dim=-1, n=1):
    "Same as `torch.unsqueeze` but can add `n` dims"
    for _ in range(n): x = x.unsqueeze(dim)
    return x

# %% ../nbs/Fastai/00_torch_core.ipynb 61
def unsqueeze_(x, dim=-1, n=1):
    "Same as `torch.unsqueeze_` but can add `n` dims"
    for _ in range(n): x.unsqueeze_(dim)
    return x

# %% ../nbs/Fastai/00_torch_core.ipynb 63
def _fa_rebuild_tensor (cls, *args, **kwargs): return cls(torch._utils._rebuild_tensor_v2(*args, **kwargs))
def _fa_rebuild_qtensor(cls, *args, **kwargs): return cls(torch._utils._rebuild_qtensor  (*args, **kwargs))

# %% ../nbs/Fastai/00_torch_core.ipynb 64
...
# %% ../nbs/Fastai/00_torch_core.ipynb 65
...
# %% ../nbs/Fastai/00_torch_core.ipynb 66
...
# %% ../nbs/Fastai/00_torch_core.ipynb 68
def to_half(b):
    "Recursively map lists of tensors in `b ` to FP16."
    return apply(lambda x: x.half() if torch.is_floating_point(x) else x, b)

# %% ../nbs/Fastai/00_torch_core.ipynb 69
def to_float(b):
    "Recursively map lists of int tensors in `b ` to float."
    return apply(lambda x: x.float() if torch.is_floating_point(x) else x, b)

# %% ../nbs/Fastai/00_torch_core.ipynb 70
# None: True if available; True: error if not available; False: use CPU
defaults.use_cuda = None

# %% ../nbs/Fastai/00_torch_core.ipynb 71
...
# %% ../nbs/Fastai/00_torch_core.ipynb 72
...
# %% ../nbs/Fastai/00_torch_core.ipynb 74
...
# %% ../nbs/Fastai/00_torch_core.ipynb 77
def to_cpu(b):
    "Recursively map lists of tensors in `b ` to the cpu."
    return to_device(b,'cpu')

# %% ../nbs/Fastai/00_torch_core.ipynb 79
def to_np(x):
    "Convert a tensor to a numpy array."
    return apply(lambda o: o.data.cpu().numpy(), x)

# %% ../nbs/Fastai/00_torch_core.ipynb 81
...
# %% ../nbs/Fastai/00_torch_core.ipynb 85
@patch
...
# %% ../nbs/Fastai/00_torch_core.ipynb 86
if not hasattr(torch,'as_subclass'): torch.as_subclass = torch.Tensor.as_subclass

# %% ../nbs/Fastai/00_torch_core.ipynb 87
@patch
def as_subclass(self:Tensor, typ):
    "Cast to `typ` and include `__dict__` and meta"
    return retain_meta(self, torch.as_subclass(self, typ))

# %% ../nbs/Fastai/00_torch_core.ipynb 90
...
# %% ../nbs/Fastai/00_torch_core.ipynb 91
...
# %% ../nbs/Fastai/00_torch_core.ipynb 105
class TensorImageBase(TensorBase):
    _show_args = ArrayImageBase._show_args
    def show(self, ctx=None, **kwargs):
        return show_image(self, ctx=ctx, **{**self._show_args, **kwargs})

# %% ../nbs/Fastai/00_torch_core.ipynb 106
class TensorImage(TensorImageBase): pass

# %% ../nbs/Fastai/00_torch_core.ipynb 107
class TensorImageBW(TensorImage): _show_args = ArrayImageBW._show_args

# %% ../nbs/Fastai/00_torch_core.ipynb 108
...
# %% ../nbs/Fastai/00_torch_core.ipynb 109
...
# %% ../nbs/Fastai/00_torch_core.ipynb 132
def concat(*ls):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 134
class Chunks:
...
# %% ../nbs/Fastai/00_torch_core.ipynb 139
...
# %% ../nbs/Fastai/00_torch_core.ipynb 141
class ShowTitle:
...
# %% ../nbs/Fastai/00_torch_core.ipynb 148
@patch
...
# %% ../nbs/Fastai/00_torch_core.ipynb 152
def get_empty_df(n):
    "Return `n` empty rows of a dataframe"
    df = pd.DataFrame(index = range(n))
    return [df.iloc[i] for i in range(n)]

# %% ../nbs/Fastai/00_torch_core.ipynb 153
def display_df(df):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 154
def get_first(c):
    "Get the first element of c, even if c is a dataframe"
    return getattr(c, 'iloc', c)[0]

# %% ../nbs/Fastai/00_torch_core.ipynb 155
def one_param(m):
    "First parameter in `m`"
    return first(m.parameters())

# %% ../nbs/Fastai/00_torch_core.ipynb 156
...
# %% ../nbs/Fastai/00_torch_core.ipynb 157
def find_device(b):
    "Recursively search the device of `b`."
    return item_find(b).device

# %% ../nbs/Fastai/00_torch_core.ipynb 159
def find_bs(b):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 161
def np_func(f):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 165
class Module(nn.Module, metaclass=PrePostInitMeta):
    "Same as `nn.Module`, but no need for subclasses to call `super().__init__`"
    def __pre_init__(self, *args, **kwargs): super().__init__()
    def __init__(self): pass

# %% ../nbs/Fastai/00_torch_core.ipynb 168
from torch.nn.parallel import DistributedDataParallel

# %% ../nbs/Fastai/00_torch_core.ipynb 169
def get_model(model):
    "Return the model maybe wrapped inside `model`."
    return model.module if isinstance(model, (DistributedDataParallel, nn.DataParallel)) else model

# %% ../nbs/Fastai/00_torch_core.ipynb 170
def one_hot(x, c):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 172
def one_hot_decode(x, vocab=None):
    return L(vocab[i] if vocab else i for i,x_ in enumerate(x) if x_==1)

# %% ../nbs/Fastai/00_torch_core.ipynb 174
def params(m):
    "Return all parameters of `m`"
    return [p for p in m.parameters()]

# %% ../nbs/Fastai/00_torch_core.ipynb 175
def trainable_params(m):
    "Return all trainable parameters of `m`"
    return [p for p in m.parameters() if p.requires_grad]

# %% ../nbs/Fastai/00_torch_core.ipynb 177
norm_types = (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d, nn.LayerNorm)

# %% ../nbs/Fastai/00_torch_core.ipynb 178
...
# %% ../nbs/Fastai/00_torch_core.ipynb 180
...
# %% ../nbs/Fastai/00_torch_core.ipynb 182
@patch
...
# %% ../nbs/Fastai/00_torch_core.ipynb 184
@patch
...
# %% ../nbs/Fastai/00_torch_core.ipynb 185
def logit(x):
    "Logit of `x`, clamped to avoid inf."
    x = x.clamp(1e-7, 1-1e-7)
    return -(1/x-1).log()

# %% ../nbs/Fastai/00_torch_core.ipynb 186
def num_distrib():
    "Return the number of processes in distributed training (if applicable)."
    return int(os.environ.get('WORLD_SIZE', 0))

# %% ../nbs/Fastai/00_torch_core.ipynb 187
def rank_distrib():
    "Return the distributed rank of this process (if applicable)."
    return int(os.environ.get('RANK', 0))

# %% ../nbs/Fastai/00_torch_core.ipynb 188
def distrib_barrier():
    "Place a synchronization barrier in distributed training"
    if num_distrib() > 1 and torch.distributed.is_initialized(): torch.distributed.barrier()

# %% ../nbs/Fastai/00_torch_core.ipynb 190
# Saving arrays requires pytables - optional dependency
try: import tables
except: pass

# %% ../nbs/Fastai/00_torch_core.ipynb 191
def _comp_filter(lib='lz4',lvl=3): return tables.Filters(complib=f'blosc:{lib}', complevel=lvl)

# %% ../nbs/Fastai/00_torch_core.ipynb 192
@patch
...
# %% ../nbs/Fastai/00_torch_core.ipynb 194
@patch
def load_array(p:Path):
    "Save numpy array to a `pytables` file"
    with tables.open_file(p, 'r') as f: return f.root.data.read()

# %% ../nbs/Fastai/00_torch_core.ipynb 195
def base_doc(elt):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 196
def doc(elt):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 197
...
# %% ../nbs/Fastai/00_torch_core.ipynb 199
...
# %% ../nbs/Fastai/00_torch_core.ipynb 202
...
# %% ../nbs/Fastai/00_torch_core.ipynb 205
...
# %% ../nbs/Fastai/00_torch_core.ipynb 208
def requires_grad(m):
    "Check if the first parameter of `m` requires grad or not"
    ps = list(m.parameters())
    return ps[0].requires_grad if len(ps)>0 else False

# %% ../nbs/Fastai/00_torch_core.ipynb 210
...
# %% ../nbs/Fastai/00_torch_core.ipynb 212
def cond_init(m, func):
    "Apply `init_default` to `m` unless it's a batchnorm module"
    if (not isinstance(m, norm_types)) and requires_grad(m): init_default(m, func)

# %% ../nbs/Fastai/00_torch_core.ipynb 214
def apply_leaf(m, f):
...
# %% ../nbs/Fastai/00_torch_core.ipynb 216
def apply_init(m, func=nn.init.kaiming_normal_):
    "Initialize all non-batchnorm layers of `m` with `func`."
    apply_leaf(m, partial(cond_init, func=func))

# %% ../nbs/Fastai/00_torch_core.ipynb 219
...
# %% ../nbs/Fastai/00_torch_core.ipynb 220
...
# %% ../nbs/Fastai/00_torch_core.ipynb 225
...
# %% ../nbs/Fastai/00_torch_core.ipynb 226
