...
__all__ = ['IN_IPYTHON', 'Event', 'isevent', 'EVENT_ORDER_MAPPING', 'PREFIXES', 'event', 'events', 'Loop', 'Outer',
           'Inner', 'FailingInner']
...
class _Event(dp.iter.IterDataPipe):
    def __init__(self, source_dp: (dp.iter.IterDataPipe,'Loop'), fn) -> None:
        super().__init__()
...
class Loop(object):
    ...
    def build(self):
        ...
        chain = L(inspect.getmembers(self)).map(Self[1]).filter(isevent)

        if len(chain)==0: return
        self.datapipe = chain[0](self)
        if len(chain)>1:
            for _event in chain[1:]: self.datapipe = _event(self.datapipe)
