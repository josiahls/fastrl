...
__all__ = ['Callback', 'filter_call_on_cbs', 'set_cbs', 'filter_exclude_under_cbs', 'callback_iter', 'callback_getitem',
           'default_constructor']
...
def callback_iter(f):
    ...
    def _inner(self):
        try:
            compose(*self.cb_before)()
            for record in f(self):
                compose(*self.cb_on)()
                yield record
            compose(*self.cb_after)()
        except Exception:
            compose(*self.cb_failed)()
        finally:
            compose(*self.cb_finally)()
    return _inner
